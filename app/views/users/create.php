...
<body class="bg-surface text-on-surface">
    ...
    <main class="min-h-screen bg-surface p-8 pt-24 lg:ml-64">
        <div class="mx-auto max-w-4xl space-y-6">
            <div>
                <h1 class="text-4xl font-extrabold tracking-tight text-on-surface">เพิ่มผู้ใช้</h1>
                <p class="mt-2 text-on-surface-variant">สร้างบัญชีผู้ใช้ใหม่และกำหนดสิทธิ์การเข้าถึงระบบ</p>
            </div>
                ...
                <form method="POST" action="<?= htmlspecialchars(base_path_url('/users/store')) ?>">
                    <div class="grid grid-cols-1 gap-6 p-8 md:grid-cols-2">
                        <div class="flex flex-col gap-1.5">
                            ...
                            <input class="rounded-lg border border-outline-variant/20 bg-white px-4 py-3 text-on-surface focus:outline-none focus:ring-2 focus:ring-primary/50" type="text" name="name" value="<?= htmlspecialchars($old['name'] ?? '') ?>" required>
                        </div>
                        <div class="flex flex-col gap-1.5">
                            <label class="text-xs font-bold uppercase tracking-widest text-on-surface-variant">อีเมล</label>
                            <input class="rounded-lg border border-outline-variant/20 bg-white px-4 py-3 text-on-surface focus:outline-none focus:ring-2 focus:ring-primary/50" type="email" name="email" value="<?= htmlspecialchars($old['email'] ?? '') ?>" required>
                        </div>
                        <div class="flex flex-col gap-1.5">
                            <label class="text-xs font-bold uppercase tracking-widest text-on-surface-variant">รหัสผ่าน</label>
                            <input class="rounded-lg border border-outline-variant/20 bg-white px-4 py-3 text-on-surface focus:outline-none focus:ring-2 focus:ring-primary/50" type="password" name="password" required>
                        </div>
                        <div class="flex flex-col gap-1.5">
                            <label class="text-xs font-bold uppercase tracking-widest text-on-surface-variant">ยืนยันรหัสผ่าน</label>
                    ...
                    <div class="flex justify-end gap-4 bg-surface-container-low p-6">
                        ...
                        <button class="rounded-lg bg-gradient-to-r from-primary to-secondary px-6 py-2 font-bold text-white shadow-panel transition-all hover:brightness-110 active:scale-95" type="submit">บันทึกผู้ใช้</button>
                    </div>
                </form>
            </section>
        </div>
